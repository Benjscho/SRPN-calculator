...
def overflow_check(value):
    """
    This function checks whether the value is within the overflow in the original srpn calculator. If it is outside
    that range, the saturated value is returned.
    """
    if value > 2147483647:
        value = 2147483647
...
def exceptional_parse(input_to_parse):
        ...
        if j.isdigit():
            ...
            if temp_number != "":
                if negative_flag:
                    ...
                    temp_nums.append(temp_number)
                    temp_number = ""
                # This logic seems to solve some edge cases, while failing others. Needs further troubleshooting.
                if len(temp_nums) > 2:
                    temp_exp += temp_nums
                    temp_exp += temp_ops[::-1]
                    temp_nums = []
            ...
            # If character is in the legal operators, add to the temporary operators
            if j in legal_operators:
                temp_ops.append(j)
            # This logic also is required for some edge cases.
            elif j == "=":
                temp_exp += temp_nums
                temp_exp.append(j)
                temp_exp += temp_ops[::-1]
                temp_nums = []
                temp_ops = []
            # Provide handling for comment flags in continuous strings (these are unrecognised in original SRPN)
            elif j == "#":
    ...
    # Logic to reconcile any temporary number, and concatenate the final split expression to be returned and parsed.
    if temp_number != "":
        temp_nums.append(temp_number)
...
def parse(element):
    ...
    '''
    First the code checks if this element is flagged as an octal. Numbers are
    ...
    The difficulty is a bad octal (e.g., 0109) is ignored by the SRPN. So
    ...
    if len(octal_check) > 1:
        ...
        if octal_boolean[0] or octal_boolean[1]:
            try:
    ...
    # this will print the 23 randoms that are always produced by the SRPN to
    # emulate. After that a random int between 1 and 2147483647 is returned.
    # I believe the original function is rand() from the C++ standard library
    # but there was no easy way for me to port that.
    elif element == 'r':
...
def operands(val1, val2, operator):
    ...
    """
    # This is exceptional handling for when the power is 0. Even if val2 is negative, the result is always positive 1
    # in the original srpn.
    if operator == "**" and val1 == 0:
        val2 = 1
...
def test_main(calculation):
    """
    Function that imitates the main function on a test calculation input, called by test_srpn.py.
    """
